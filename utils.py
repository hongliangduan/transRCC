...
def copy_model_parameters(model1, model2, factor=1):
    
    m1_params = [t for t in tf.trainable_variables() if t.name.startswith(model1.scope)]
    m1_params = sorted(m1_params, key=lambda v: v.name)
    m2_params = [t for t in tf.trainable_variables() if t.name.startswith(model2.scope)]
    m2_params = sorted(m2_params, key=lambda v: v.name)

    update_ops = []
    for m1_v, m2_v in zip(m1_params, m2_params):
