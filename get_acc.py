...
import os
road = r'data/out/smiles/RCC_key1'
list1 = os.listdir(road)
f1 = open(str(road + '/' + 'test.target'), 'r')
list2 = f1.readlines()
f2 = open(str(road + '/'+'accuracy.txt'), 'w')
f5 = open(str(road + '/' + 'standard_test.target'), 'w')
number1 = 0
number2 = 0
number3 = 0
j = 1
for dirname in list1:
    if '.out' in dirname and 'standard_' not in dirname:
        file1 = str(road + '/' + dirname)
        file2 = str(road + '/standard_' + dirname)
        f3 = open(file1, 'r')
        f4 = open(file2, 'w')
        l1 = f3.readlines()
        ...
        f2.write(str(j) + '、'+str('标准化前' + dirname + '和标准化前的test.target相比准确率是' + str(accuracy1)) + '\n'
                 + str(j) + '、'+'标准化前' + str(dirname) +
                 '和标准化后的test.target相比准确率是'+str(accuracy2)+'\n'
