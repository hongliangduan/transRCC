...
parser.add_argument('--model_name',
                    ...
                    default='ChemTrm',
                    help='name of model')
parser.add_argument('--approx_num',
                    action='store',
                    type=int,
                    default=3144,
...
def learning_rate_factor(name,
    ...
    if name == "constant":
        ...
        in_warmup = tf.cast(step_num <= learning_rate_warmup_steps,
                            dtype=tf.float32)
        ret = 0.5 * (1 + tf.cos(np.pi * step_num / learning_rate_decay_steps))
        ...
        return scale * tf.rsqrt(
            tf.maximum(step_num, learning_rate_warmup_steps))
    elif name == "exp_decay":
...
def eval(test_model, sess, writer, tf_acc, acc_summary, global_step, val_data,
         max_length, approx_num, code_of_start, code_of_end, code_of_pad):
...
if __name__ == '__main__':
    ...
    lr = learning_rate_schedule(lr_schedule, global_step, hidden_size=emb_dim)
    ...
    def build_model_fn_wrapper(reuse=tf.AUTO_REUSE, is_train=True):
        model = build_model_fn(Transformer,
                               **kwargs,
                               reuse=reuse,
                               is_train=is_train,
                               lr=lr,
    ...
    for epoch in range(max_epochs):
        ...
        while True:
            ...
            batch_targets = batch_pad(batch_targets,
                                      val=vocab_set[sign_pad],
                                      front=False)

            loss, np_lr, train_summary = model.update(sess, batch_inputs,
                                                      batch_targets)
            ##########
            # split to all gpus
            # try:
        ...
        over_print('{}/{}, mean_loss={}'.format(pos + new_batch_size, N,
                                                np.mean(losses)))
        print()
        ...
        eval(test_model, sess, writer, tf_acc, acc_summary, global_step,
             val_data, max_sent_len, approx_num, vocab_set[sign_start],
             vocab_set[sign_end], vocab_set[sign_pad])
